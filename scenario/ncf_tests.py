...
if re.match(r"^[0-9]+$", tag):
  ...
  if redmine_branch == "master":
    ...
    agent_version = get_issue_rudder_version(issue)
  else:
...
# Testing a ncf version
  ...
  if not cfengine_version:
    cfengine_version = "rudder-" + tag
  test_shell_on("agent", export_prefix + ";sh /tmp/ncf-setup test-local https://github.com/Normation/ncf.git#branches/rudder/" + tag + " " + cfengine_version + " --testinfra", live_output=True)
