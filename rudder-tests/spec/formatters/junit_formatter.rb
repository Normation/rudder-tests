# Taken for the most part from the https://github.com/natritmeyer/yarjuf project
...
class JUnit
  ...
  def self.root_group_name_for(example_notification)
    File.basename(example_notification.example.metadata[:file_path], ".rb") + " => " + example_notification.example.metadata[:subject].to_s
  end
  ...
  def build_test(test)
    test_name      = test.metadata[:example_group][:description_args].join(", ") + " " + test.metadata[:description]
